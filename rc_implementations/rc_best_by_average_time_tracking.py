...
# 根据汽车的位置、目的地、剩余能量等信息，选择一个最优的充电站。
class RCBestByAverageTime(RecommendationCenter):
    def recommend_charging_station(self, car):
        ...
        selected_charging_station_occupation = None
        selected_charging_station_two_way_distance = None

        # 遍历所有充电站，计算每个充电站到达时间、离开时间、距离、当前占用情况等信息；
        for charging_station in self.charging_stations:
            if car.car_number == 1:
                print("AVG ", charging_station.charging_station_number, charging_station.get_number_of_free_spots(),
                      charging_station.get_queue_size())
            # 根据充电站的平均充电时间、平均等待时间等信息，估算在该充电站充电的总时间以及总的无效时间；
            ...
            distance_to_destination = abs(car.destination[0] - car.x_coordinate) + abs(
                car.destination[1] - car.y_coordinate)
            current_occupation = self.get_cs_occupation(charging_station.charging_station_number,
                                                        self.env.now + one_way_distance)
            current_fast_spot_occupation = self.get_fast_spot_occupation(charging_station.charging_station_number,
                                                                         self.env.now + one_way_distance)

            occupation_list = self.get_cs_arrival_and_departure_list(charging_station.charging_station_number,
                                                                     self.env.now + one_way_distance)
            fast_spot_occupation_list = self.get_fast_spots_arrival_and_departure_list(
                charging_station.charging_station_number, self.env.now + one_way_distance)
            ...
            approx_unprod_time = two_way_distance + \
                                 (self.get_avg_charging_time_for_cs(charging_station.charging_station_number) * current_occupation)\
                                 + (self.get_avg_waiting_time_for_cs(charging_station.charging_station_number) *
                                    (current_occupation if current_occupation > self.capacity else self.capacity - self.capacity))

            if selected_charging_station is None or approx_unprod_time < selected_charging_station_approx_unprod_time:
                ...
                selected_charging_station_occupation = current_occupation
                selected_charging_station_two_way_distance = two_way_distance
                selected_charging_station_approx_unprod_time = approx_unprod_time
        ...
        if not car.nearest:
            self.future_cs_arrivals[selected_charging_station.charging_station_number].append(
                selected_charging_station_arrival_time)
            ...
            if selected_charging_station_fast_charge:
                self.future_fast_spots_arrivals[selected_charging_station.charging_station_number].append(
                    selected_charging_station_arrival_time)
                ...
                self.cars_arriving_fast_spots_list[selected_charging_station.charging_station_number].append(
                    car.car_number)
        ...
        #        print("Fast:", selected_charging_station_fast_charge)
        if car.car_number == 1:
